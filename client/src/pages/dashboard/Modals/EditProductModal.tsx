    ...
    const save = async (newProduct) => {
        ...
        try {
            ...
            if (newProduct.image[0]) {
                ...
                // Replacing Axios with Fetch for image upload
                response = await fetch("/api/products/product/upload", {
                    method: "POST",
                    headers: {
            ...
            // Replacing Axios with Fetch for product update
            const resp = await fetch("/api/products/product/" + id, {
                ...
                body: JSON.stringify({ ...newProduct, image: imageName || product.image })
            });
